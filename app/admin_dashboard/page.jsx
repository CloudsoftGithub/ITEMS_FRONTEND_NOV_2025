...
{ name: "Academic", icon: <Calendar className="w-4 h-4 mr-2" /> },
...
default:
return ( <div className="p-6"> <h2 className="text-2xl font-semibold">Admin Dashboard</h2> <p className="mt-2 text-gray-600">Select a section from the sidebar.</p> <p className="mt-4 text-sm text-gray-600">
Signed in as: <span className="font-medium">{user?.username}</span> </p> </div>
);
}
};

return ( <ProtectedRoute>
{/* MOBILE NAVBAR */} <div className="md:hidden flex items-center justify-between px-4 py-3 bg-white border-b shadow-sm">
<button onClick={() => setSidebarOpen(true)}> <Menu className="w-6 h-6" /> </button> <h1 className="text-xl font-semibold">Admin Dashboard</h1> </div>
  ...
  <div className="flex min-h-screen">
    ...
    >
      ...
      <div className="md:hidden flex justify-end mb-4">
        <button className="text-gray-600 hover:text-black" onClick={() => setSidebarOpen(false)}>
          Close ✕
        </button>
      </div>
    ...
    {/* Main content */}
    <main className="flex-1 bg-gray-100 p-4 md:p-6 overflow-auto">{renderContent()}</main>
  </div>
</ProtectedRoute>
